            ...
            <table class="table table-striped table-bordered table-hover table-condensed">
				...
				<#if s.result.class.simpleName == "TierBar">
					<#if s.result.matched != s.result.outOf>
						<td colspan="5"><span class="text-muted">Results that match ${s.result.matched} of ${s.result.outOf} words</span></td>
                    <#else>
                        <td colspan="5" class="hidden">Fully-matching results</td>
					</#if>
				<#else>                                        
                    <#-- EACH RESULT -->					
                    <td><a href="${s.result.clickTrackingUrl?html}" title="${s.result.liveUrl?html}"><@s.boldicize><@s.Truncate length=70>${s.result.title}</@s.Truncate></@s.boldicize></a></td>
                    <td><#if s.result.date?? && s.result.date?date?string("d MMM yyyy") != "No Date"><span class="text-muted">${s.result.date?date?string("d MMM yyyy")}</span></#if></td>
                    <td><#if s.result.summary??><@s.boldicize>${s.result.summary}</@s.boldicize></#if></td>
                    <td>${s.result.fileType}</td>
                    <td>
                        <#-- SECONDARY ACTIONS -->
    					<div class="btn-group">
							<a href="#" class="dropdown-toggle" data-toggle="dropdown" title="More actions&hellip;"><small><i class="glyphicon glyphicon-chevron-down"></i></small></a>
